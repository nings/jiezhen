...
from src.strategies.zhen2_strategy import Zhen2Strategy

# 初始化日志
logger = Logger.get_logger(__name__, log_file="log/okx2.log")
...
def main():
    ...
    try:
        # 加载配置
        logger.info("=" * 80)
        logger.info("Jiezhen 接针策略 启动 (版本2)")
        logger.info("=" * 80)
        ...
        # 初始化策略
        strategy = Zhen2Strategy(
            client=client,
            instrument_manager=instrument_manager,
            order_manager=order_manager,
            notification_manager=notification_manager
        )
        logger.info("策略初始化成功 - Zhen2Strategy (使用平均值方法)")
